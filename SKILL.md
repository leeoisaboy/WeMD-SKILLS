...
description: Create Chinese illustrations for WeChat/WeMD markdown articles with Alibaba Cloud Qwen Image 3.0, Seedream 5.0 as the first fallback, and HTML diagrams only as the last resort. Save images into per-week folders, upload them to the official img.wemd.app image host, and replace local image paths with https://img.wemd.app/... URLs. Use when generating article illustrations or concept images for WeMD/WeChat articles, including multi-image coherent illustration sets.
---
...
3. If Qwen is unavailable or fails, fall back to Seedream:
...
4. Only if both Qwen and Seedream fail or are unavailable, use the HTML renderer as the last resort:
...
5. Verify: generated PNGs exist and are nonblank, every `img.wemd.app` URL returns HTTP 200 for `image/*`, and the markdown contains no local image paths.
